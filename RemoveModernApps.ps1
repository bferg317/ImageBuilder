...
function Get-AppList
{
  begin
  {
      # Build list of applications to remove
      Write-Verbose "Building list of applications to remove"
      $list = @()
      
	  # Get full list of installed apps with "Get-AppxProvisionedPackage -Online" use DisplayName
	  $list += "Microsoft.3DBuilder"
	  $list += "Microsoft.Getstarted"
	  $list += "microsoft.windowscommunicationsapps"
	  $list += "Microsoft.MicrosoftOfficeHub"
	  $list += "Microsoft.Messaging"
	  $list += "Microsoft.XboxApp"
	  $list += "Microsoft.SkypeApp"
	  $list += "Microsoft.Office.OneNote"
